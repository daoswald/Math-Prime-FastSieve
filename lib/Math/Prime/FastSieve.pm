## no critic(RCS)
#           (Git doesn't place refuse in version controlled files.)
...
Sieve::Sieve( long n ) : num_primes(0), max_n(0), sieve(NULL)
{
...
//    num_primes = 0;
    ...
    {
        max_n = static_cast<std::vector<bool>::size_type>(n);
        for( std::vector<bool>::size_type i = 3; i * i <= max_n; i+=2 )
            if( ! (*primes)[i] )
                for( std::vector<bool>::size_type k, j=i; (k=i*j) <= n; j+=2 )
                    (*primes)[k] = 1;
    }
    sieve = primes;
}
...
unsigned long Sieve::nearest_le( long n )
{
    ...
    while( n_index > 2 )
    {
        if( ! (*sieve)[n_index] ) return static_cast<unsigned long>(n_index);
...
unsigned long Sieve::nearest_ge( long n )
{
    ...
    while( n_idx <= max_n )
    {
        if( ! (*sieve)[n_idx] ) return static_cast<unsigned long>(n_idx);
...
{
    if( n <  1     ) return 0UL; // Why would anyone want the 0th prime?
    if( n >  max_n ) return 0UL; // There can't be more primes than sieve.
    if( n == 1     ) return 2UL; // We have to handle the only even prime.
